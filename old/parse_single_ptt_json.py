...
def parse_ptt_article(target_url):
    ...
    try:
        ...
        print(f"URL 解析時間失敗: {target_url}, Error: {e}")
        return None # 或給個預設值
        ...
        if msgTag and msgAuthor and msgContent and msgTime_pre:
            ...
            if len(msgTime_pre_list) > 2: # 有 ip 版本 (會有 ip、日期、時間)
                ...
                if len(metadataList) > 0: # 如果有標題作者欄位，可與推文時間比較
                    msgTime = str(article_time.year) + "/" + origin_msgTime # 跨年度邏輯驗證
                    if article_time > datetime.datetime.strptime(msgTime, "%Y/%m/%d %H:%M").replace(tzinfo= timezone_tw):
                        msgTime = str(int(article_time.year) + 1) + "/" + origin_msgTime
                        msgTime = datetime.datetime.strptime(msgTime, "%Y/%m/%d %H:%M").replace(tzinfo= timezone_tw)
                    else:
                        msgTime = datetime.datetime.strptime(msgTime, "%Y/%m/%d %H:%M").replace(tzinfo= timezone_tw)
                else: # 如果沒有標題作者欄位，則不與推文時間比較
                    msgTime = str(article_time.year) + "/" + origin_msgTime
                    msgTime = datetime.datetime.strptime(msgTime, "%Y/%m/%d %H:%M").replace(tzinfo= timezone_tw)
            else: # 無 ip 版本
                origin_msgTime = " ".join(msgTime_pre_list)
...
article_json = parse_ptt_article(target_url)

with open(output_folder, 'w', encoding='utf-8') as json_file:
    # json.dump() 將字典寫入檔案物件
    # indent=4 讓 JSON 格式化，有縮排，更易讀
    # ensure_ascii=False 確保中文字符能正確顯示 (避免亂碼)
    json.dump(my_data, json_file, ensure_ascii=False, indent=4)
    
print(f"字典已儲存至 {article_json}")
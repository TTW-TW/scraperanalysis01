...
wordClean = {}
for word in s2_list:
    if (word not in skipWord) and (len(word.strip()) != 0):
        if word not in wordClean:
            wordClean[word] = 1
        else:
            wordClean[word] += 1
...
sorted_wordClean = sorted(wordClean.items(), reverse=True, key = lambda x:x[1])
